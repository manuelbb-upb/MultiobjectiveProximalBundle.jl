...
function solve_dual!(
	...
	# that `-1/weight * p ≈ value.(d)`, where `p` is the solution
	...
	@objective(model, Min, ν + 0.5 * weight * sum(d.^2))
	@constraint(model, -err_f_p .+ sg_f_p*d .<= ν)
	@constraint(model, -err_g_p .+ sg_g_p*d .<= ν)
	for j in _J
		@constraint(model, -err_f[:,j] .+ sg_f[:,:,j]*d .<= ν)
		@constraint(model, -err_g[:,j] .+ sg_g[:,:,j]*d .<= ν)
	end
